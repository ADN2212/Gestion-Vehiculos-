...
@api_view(['GET'])
@csrf_exempt
@permission_classes([AllowAny])#Esta es la que está por default, pero la pongo para ser descriptivo.
def  get_choferes(request, id):
	...
	if request.method == 'GET':
		...
		if id == 'all':
			...
			return Response([]);#Si no hay choferes es mejor que retornr un array vacío 
